...
class WaServiceTest {
    ...
    private val receipt =
        ReceiptParams("FMU-0915-001", "Familia Urban", "Cuci Setrika 4,5 kg", "Rp31.000", "Tunai", "300", "1.140")
    ...
    fun `should confirm consent on the first order after opting in, then send the receipt`() {
        ...
        service.queueForPaidOrder(FAMILIA_URBAN.id, orderId, dewi, receipt) shouldBe true
    ...
    fun `should send only the receipt once consent was confirmed`() {
        ...
        service.queueForPaidOrder(FAMILIA_URBAN.id, orderId, dewi, receipt)
    ...
    fun `should queue nothing at all for a customer who is not opted in now`() {
        ...
        service.queueForPaidOrder(FAMILIA_URBAN.id, orderId, notOptedIn, receipt) shouldBe false
        service.queueReadyForPickup(
            FAMILIA_URBAN.id,
            orderId,
            notOptedIn,
            "FMU-0915-001",
